...
import com.example.productcatalogservice_jul2024.Dto.FakeStoreProductDto;
...
public class FakeStoreProductService implements IProductService {
    ...
    public List<Product> getAllProducts() {
        RestTemplate restTemplate = restTemplateBuilder.build();//1
        FakeStoreProductDto[] fakeStoreProductDtos =
                restTemplate.getForEntity("https://fakestoreapi.com/products/",
                        FakeStoreProductDto[].class).getBody();//2
        List<Product> products = new ArrayList<>();//3
        for(FakeStoreProductDto fakeStoreProductDto : fakeStoreProductDtos) {
            products.add(getProduct(fakeStoreProductDto));//4
        }
    ...
    public Product replaceProduct(Long id, Product product) {
        ...
        FakeStoreProductDto fakeStoreProductDtoResponse = requestForEntity(HttpMethod.PUT,"https://fakestoreapi.com/products/{id}",
                fakeStoreProductDto,FakeStoreProductDto.class,id).getBody();
        return getProduct(fakeStoreProductDtoResponse);
    }

    public <T> ResponseEntity<T> requestForEntity(HttpMethod httpMethod,
    ...
    private Product getProduct(FakeStoreProductDto fakeStoreProductDto) {//5
